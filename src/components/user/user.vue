<template>
  <div>
    <div>
      ...
      <el-button type="warning" icon="el-icon-view" round @click="isUpdataPassword()">修改密码</el-button>      
    </div>
    <el-table
      ref="singleTable"
      :data="tableData"
    ...
    <!-- 修改密码弹窗 -->
      <el-dialog :title="username" :visible.sync="passwordVisible">
...
<script type="text/acmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      UpdataPassword () {
        if(this.passWordform.password == '') {
          this.feedbackMessage("密码不能为空",'error')
        } else {
            ...
            //请求结果
            this.feedbackMessage("修改密码成功",'success')
        }
    ...
    computed: {
      //修改密码弹出框中用户名字的获取和拼接
      username: function () {
        var name = (this.currentRow == null) ? '':this.currentRow.name;
        return "修改" + name+ "的登录密码";
      }
    }
  }
</script>
